	...
	public static void main(String[] args) {
		...
		while (loopOn) {
			...
			if(escsoft.size() == 1){
				...
				objexibir.exibirMin(escsoft.get(0), cpubd.getMinCPUBD(reccpu.getRecCPUBD(escsoft.get(0))), gpubd.getMinGPUBD(recgpu.getRecGPUBD(escsoft.get(0))), rambd.getMinRAMBD(recram.getRecRAMBD(escsoft.get(0))));
				objexibir.exibirRec(escsoft.get(0), cpubd.getRecCPUBD(reccpu.getRecCPUBD(escsoft.get(0))), gpubd.getRecGPUBD(recgpu.getRecGPUBD(escsoft.get(0))), rambd.getRecRAMBD(recram.getRecRAMBD(escsoft.get(0))));
			}
			if(escsoft.size() > 1){
